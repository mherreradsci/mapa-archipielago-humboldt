...
# Full run: both raster sources + HTML (slow: downloads ~5000 tiles, needs ~4 GB RAM)
...
4. **Map composition (matplotlib, Agg backend)** — `componer_lamina()` assembles the A0 sheet: image + graticule (every 0.1°), latitude-corrected scale bar, north arrow, title, and the source attribution footer. Font sizes are in points (independent of DPI), derived from `fs_base = 26`.
